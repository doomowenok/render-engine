                ...
                tex2_t a_uv, tex2_t b_uv, tex2_t c_uv)
{
    ...
    if(interpolated_reciprocal_w < get_z_buffer_at(x, y))
    {
        // Get the buffer of colors of the texture
...
vec3_t get_triangle_normal(vec4_t v[3])
{
    vec3_t vector_a = vec3_from_vec4(v[0]); /*   A   */
    vec3_t vector_b = vec3_from_vec4(v[1]); /*  / \  */
    vec3_t vector_c = vec3_from_vec4(v[2]); /* C---B */
